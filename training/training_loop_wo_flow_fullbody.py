...
def denorm_clothes(norm_patches, norm_patches_lower, M_invs, norm_clothes_mask, norm_clothes_mask_lower, col, row, gnum):
    ...
    for ii in range(M_invs.shape[1]):
        ...
        if ii >= 6:
            ...
            if row < 2 * gap:
                norm_patch_lower = norm_patches_lower[col,(ii-6)*3:(ii-6+1)*3,...].transpose(1,2,0)
                norm_clothes_mask_patch_lower = norm_clothes_mask_lower[col,(ii-6)*3:(ii-6+1)*3,...].transpose(1,2,0)
            ########## gap-(gum-1)换上衣，裤子的索引是row
            else:
                norm_patch_lower = norm_patches_lower[row,(ii-6)*3:(ii-6+1)*3,...].transpose(1,2,0)
                norm_clothes_mask_patch_lower = norm_clothes_mask_lower[row,(ii-6)*3:(ii-6+1)*3,...].transpose(1,2,0)
        ...
        if ii >= 6:
            denorm_patch_lower = cv2.warpPerspective(norm_patch_lower,M_inv,(256,256),borderMode=cv2.BORDER_CONSTANT)
...
def setup_snapshot_image_grid(training_set, device, random_seed=0):
    ...
    # Load data.
    images, poses, norm_img, norm_img_lower, _, _, M_invs, _, _, _, norm_clothes_mask, norm_clothes_mask_lower, retain_masks = zip(*[training_set[i] for i in grid_indices])
    ...
    grid_denorm_upper_ims, grid_denorm_lower_ims, grid_denorm_upper_masks, grid_denorm_lower_masks = zip(*[denorm_clothes(norm_patches, norm_patches_lower, \
                                                                                                        M_invs, norm_clothes_mask, norm_clothes_mask_lower, \
                                                                                                        i % gnum, i // gnum, gnum) for i in range(gnum*gnum)])
    ...
    norm_img = np.concatenate([norm_img, norm_img_lower], axis=1)
...
def training_loop(
    ...
    if rank == 0:
        ...
        c = torch.empty([batch_gpu, (10*3+4*3), 64, 64], device=device)
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            ...
            # phase_norm_img = np.concatenate([phase_norm_img,phase_norm_img_lower],axis=1)
